...
// ─── Individual scroll-detect section (memoized for performance) ────────────
const Section = memo(function Section({ id, onEnter, children }) {
    ...
    return (
        <div ref={ref} id={id} className="relative w-full">
            ...
            <div className="absolute top-0 left-0 right-0 h-px z-20 pointer-events-none bg-gradient-to-r from-transparent via-red-500/20 to-transparent" />
            {children}
        </div>
    );
});

// ─── Fixed full-screen background (optimized with CSS transitions) ──────────
const SiteBackground = memo(function SiteBackground({ activeSection }) {
    return (
        <>
            {/* Base color layer - CSS transition instead of framer-motion for performance */}
            <div
                className="fixed inset-0 z-[-10] pointer-events-none transition-colors duration-700 ease-out"
                style={{ backgroundColor: SECTION_BG[activeSection]?.color ?? '#000000' }}
            />

            {/* Pattern layer - simplified with CSS opacity transition */}
            ...
            {/* Radial glow layer - CSS transition */}
            <div
                className="fixed inset-0 z-[-1] pointer-events-none transition-all duration-700 ease-out"
                style={{ background: GLOWS[activeSection] ?? 'none' }}
            />
        </>
    );
});

// ─── Section indicator dots (optimized with CSS animations) ──────────────────
const SectionDots = memo(function SectionDots({ activeSection }) {
    ...
    return (
        <div className="fixed right-5 top-1/2 -translate-y-1/2 z-50 flex-col gap-4 pointer-events-none hidden lg:flex">
            {visible.map((id) => {
                ...
                return (
                    ...
                    >
                        ...
                        {/* Dot - CSS only for performance */}
                        <div
                            className={`rounded-full transition-all duration-300 ${
                                isActive
                                    ? 'w-6 h-2 bg-red-500 shadow-[0_0_12px_rgba(220,38,38,0.8)]'
...
const HomePage = ({ onReset }) => {
    ...
    // Scroll progress with throttling for performance
    useEffect(() => {
        let ticking = false;
        const onScroll = () => {
            if (!ticking) {
    ...
    return (
        <>
            {/* Top scroll progress bar - CSS only */}
            <div className="fixed top-0 left-0 right-0 h-0.5 z-[100] pointer-events-none bg-white/5">
                <div
                    className="h-full bg-gradient-to-r from-red-600 via-red-500 to-red-400 shadow-[0_0_6px_rgba(220,38,38,0.6)] transition-[width] duration-75"
            ...
            {/* Active section watermark - simplified */}
            <div className="fixed bottom-8 left-8 z-40 pointer-events-none hidden lg:flex items-center gap-2">
                <div className="w-4 h-px bg-red-500/40" />
                <span className="text-[10px] font-mono text-white/15 uppercase tracking-[0.3em] transition-opacity duration-300">
